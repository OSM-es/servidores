...
function downloadosm {
    CURL_STATUS_CODE=$(curl -L -b /db/cookie.jar -o "/db/$2" -w "%{http_code}" "$1")
...
function updateosm {
    echo "Actualiza península1"
    OVERPASS_DIFF_URL="${OVERPASS_DIFF_URL_ESP}" /app/bin/update_overpass.sh -O /db/planet.osm.bz2 && (
        [ -s /db/replicate_id ] && mv /db/replicate_id /db/replicate_id_esp
        [ -d /db/diffs ] && mv /db/diffs /db/diffs_esp
        echo "Actualiza Canarias1"
        OVERPASS_DIFF_URL="${OVERPASS_DIFF_URL_CAN}" /app/bin/update_overpass.sh -O /db/planet.osm.bz2
    )
    STATUS=$?
...
if [[ ! -f /db/init_done ]]; then
    ...
    if [[ "$OVERPASS_MODE" = "init" ]]; then
        downloadosm "${OVERPASS_PLANET_URL_ESP}" "spain-latest-internal.osm.pbf"
        if [[ $CURL_STATUS_CODE = "200" || $CURL_STATUS_CODE = "000" ]]; then
            downloadosm "${OVERPASS_PLANET_URL_CAN}" "canarias-latest-internal.osm.pbf"
        fi
        # for `file:///` scheme curl returns `000` HTTP status code
        if [[ $CURL_STATUS_CODE = "200" || $CURL_STATUS_CODE = "000" ]]; then
            (
                osmium merge -v /db/spain-latest-internal.osm.pbf /db/canarias-latest-internal.osm.pbf -o /db/spain-full.osm.pbf
                osmium cat -v db/spain-full.osm.pbf -o /db/planet.osm.bz2
                rm /db/*.osm.pbf
